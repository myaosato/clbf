...
(defun make-token-info (&key token pos)
  (list :token (if token (string-upcase (coerce (reverse token) 'string)) nil) :pos pos))

(defun get-token (str pos &optional token)
  (if (>= pos (length str)) 
      (return-from get-token (make-token-info :token token :pos pos)))
  (let* ((ch (char str pos))
         (code (char-code ch)))
    (if (and (< 32 code) (< code 127)) ;; ASCII graphic character
        ...
        (if (eq token nil)
            ...
            (make-token-info :token token :pos pos))))) ;; return token and position
...
(defun 2bf-do (program &key (in-stream *standard-input*) (out-stream *standard-output*))
  (with-open-stream (p (make-string-input-stream (2bf program)))
    (bf-with-in p :in-stream in-stream :out-stream out-stream)))
